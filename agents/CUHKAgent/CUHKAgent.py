...
class CUHKAgent(nenv.AbstractAgent):
    ...
    def BidToOffer(self, t: float):
        ...
        if self.discountingFactor == 1 and self.maximumOfBid > 3000:
            ...
            # For small domains or when there's discounting, use time-based concession
            # Special handling when discountingFactor == 1 to avoid mathematical collapse
            if self.discountingFactor == 1:
                # Linear time-based concession for small domains
    ...
    def estimateRoundLeft(self, opponent: bool, t: float):
        ...
        time_per_round = self.maximumTimeOfOpponent + self.maximumTimeOwn

        if time_per_round == 0:
            # Cannot estimate rounds yet - return large number indicating many rounds potentially left
            return float('inf')

        round = (self.totalTime - t * self.totalTime) // time_per_round
