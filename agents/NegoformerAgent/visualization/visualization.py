...
class NegoformerVisualizationMixin:
    def plot_prediction_schedule(self):
        ...
        ax.set_title(
            ...
            f'Frequency: every {forecast_frequency} rounds | '
        ...
        # Save text mapping so it is easy to inspect exact timestep ranges.
        summary_path = f'{self.plot_dir}/prediction_schedule_summary.txt'
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write(f'total_predictions={total_predictions}\n')
            f.write(f'forecast_frequency_rounds={forecast_frequency}\n')
            f.write(f'max_predictions_target={self.max_predictions}\n')
            f.write(f'horizon={self.horizon}\n')
            f.write(f'aci_buffer={self.aci_buffer}\n')
            f.write(f'evaluation_offset=[{eval_offset_start}, {eval_offset_end})\n')
            f.write('\n')
            f.write('per_prediction_windows\n')
            for idx, prediction_round in enumerate(prediction_rounds):
                eval_start = prediction_round + eval_offset_start
                eval_end_exclusive = prediction_round + eval_offset_end
                f.write(
                    f'P{idx}: made_at_round={prediction_round}, '
                    f'evaluated_rounds=[{eval_start}, {eval_end_exclusive})\n'
                )
    ...
    def plot_observation_intervals(self, opponent_name: str = "Opponent"):
        ...
        ax_width.plot(rounds, upper_interval_widths, color='blue', linewidth=1.6, label='Upper interval width')
    ...
    def plot_residual_distribution(self, opponent_name: str = "Opponent"):
        """
        Plot predicted vs actual values for issued vanilla ACI intervals.
        """
        history = self.aci_tracker.get_observation_history()
        if not history:
            return

        evaluated_history = [d for d in history if not bool(d.get('ignored', False))]
        if not evaluated_history:
            return

        rounds = [d['round'] for d in evaluated_history]
        predicted = [d['predicted'] for d in evaluated_history]
        actuals = [d['actual'] for d in evaluated_history]
        residuals = [d['residual'] for d in evaluated_history]

        fig, ax = plt.subplots(figsize=(14, 4))
        ax.plot(rounds, predicted, 'b-', alpha=0.7, linewidth=1.2, label='Predicted')
        ax.scatter(rounds, actuals, c='green', s=18, alpha=0.7, label='Actual', zorder=3)
        ...
        ax.set_ylabel('Utility', fontsize=10)
        ax.set_title(f'mean residual={mean_residual:.3f}, max={max_residual:.3f}')
        ax.legend(loc='upper right', fontsize=8)
        ax.grid(True, alpha=0.3)
        ax.set_ylim(0, 1)

        fig.suptitle(f'Predicted vs Actual - {opponent_name}', fontsize=14)
        plt.tight_layout()

        plot_path = f'{self.plot_dir}/predicted_vs_actual_{opponent_name}.png'
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        plt.close()
