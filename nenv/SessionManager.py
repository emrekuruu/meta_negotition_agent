...
class SessionManager:
    ...
    def __init__(self, agentA_class: AgentClass, agentB_class: AgentClass, domain_name: str, deadline_time: Optional[int], deadline_round: Optional[int], estimators: List[OpponentModelClass], loggers: List[LoggerClass]):
        ...
        # Check for oracle mode via environment variable
        import os
        oracle_mode = os.environ.get('NEGOFORMER_MODE', '').lower() == 'oracle'

        # Create agents, passing opponent preference only to NegoMixerAgent
        if agentA_class.__name__ == "NegoformerAgent" or agentA_class.__name__ == "ParetoWalkerAgent":
            self.agentA = agentA_class(
                self.prefA,
                deadline_round if deadline_time is None else deadline_time,
                [estimator(self.prefA) for estimator in estimators],
                estimated_preference=self.prefB,
                mode="oracle" if oracle_mode else "opponent_model"
            )
        else:
        ...
        if agentB_class.__name__ == "NegoformerAgent" or agentB_class.__name__ == "ParetoWalkerAgent":
            self.agentB = agentB_class(
                self.prefB,
                deadline_round if deadline_time is None else deadline_time,
                [estimator(self.prefB) for estimator in estimators],
                estimated_preference=self.prefA,
                mode="oracle" if oracle_mode else "opponent_model"
            )
        else:
